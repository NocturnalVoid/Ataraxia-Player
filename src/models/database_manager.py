...
class DatabaseManager:
    def __init__(self, db_name="ataraxia.db"):
        ...
        # Adquirir lock multiplataforma sobre la BD. Si falla porque ya
        # hay otra instancia, el SingleInstanceHandler debería haber
        # avisado antes — pero por si acaso, lo loggeamos y seguimos
        # (sin lock, la app aún funciona, solo el archivo está más
        # expuesto a manipulación externa).
        from src.utils.db_lock import DatabaseLock
        self._db_lock = DatabaseLock(self.db_path)
    ...
    def release_lock(self):
        """
        Libera el lockfile del .db. Debe llamarse durante el cierre
        ordenado de la app, antes de salir del proceso.
        """
        if hasattr(self, "_db_lock") and self._db_lock is not None:
    ...
    def update_song_metadata(self, filepath: str, title: str, artist: str, album: str,
                              track_number: int = 0, genre: str = "", year: int = 0):
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("""
                UPDATE songs
                SET title = ?, track_number = ?, genre = ?, year = ?, album_id = ?
                WHERE filepath = ?
            """, (title, track_number, genre, year, album_id, filepath))
            conn.commit()
    ...
    # ══════════════════════════════════════════════════════════════════
    # CARPETAS RAÍZ DE LA BIBLIOTECA (library_folders)
    # ══════════════════════════════════════════════════════════════════

    def add_library_folder(self, path: str):
        """Registra una carpeta como raíz de la biblioteca. Idempotente."""
        if not path:
            return
        with self.get_connection() as conn:
            cursor = conn.cursor()
    ...
    def get_library_folders(self) -> list:
        """Retorna la lista de paths registrados como carpetas raíz."""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT path FROM library_folders ORDER BY added_at")
            return [row[0] for row in cursor.fetchall()]

    def remove_library_folder(self, path: str):
        """Quita una carpeta del registro. No borra las canciones asociadas."""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM library_folders WHERE path = ?", (path,))
            conn.commit()
