...
class ConverterPanel(QWidget):
    ...
    def _extract_frame_from_video(self, video_path, second=5) -> bool:
        ...
        try:
            subprocess.run(command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=5, check=True)
    ...
    def _analyze_media_with_ffprobe(self, filepath: str) -> tuple:
        ...
        try:
            ...
            res_video = subprocess.run(cmd_video, stdout=subprocess.PIPE, text=True, timeout=2)
            ...
            res_codec = subprocess.run(cmd_codec, stdout=subprocess.PIPE, text=True, timeout=2)
            if res_codec.stdout:
            ...
            res_bitrate = subprocess.run(cmd_bitrate, stdout=subprocess.PIPE, text=True, timeout=2)
            ...
            if not bitrate_str or bitrate_str == "N/A":
                ...
                res_bitrate_fmt = subprocess.run(cmd_bitrate_fmt, stdout=subprocess.PIPE, text=True, timeout=2)
            ...
            res_duration = subprocess.run(cmd_duration, stdout=subprocess.PIPE, text=True, timeout=2)
