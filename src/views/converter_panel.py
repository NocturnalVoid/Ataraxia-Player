...
from src.utils.ffmpeg_locator import get_ffmpeg_path, get_ffprobe_path
import os
import tempfile
...
class ConverterPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Color de texto dinámico para el QProgressBar (palette(text) no se
        # evalúa bien en stylesheets de Qt en todas las plataformas)
        _txt = self.palette().color(self.foregroundRole()).name()
    ...
    def _extract_frame_from_video(self, video_path, second=5) -> bool:
        ...
        command = [
            get_ffmpeg_path(), "-y", "-i", video_path, "-ss", f"00:00:0{second}", 
    ...
    def _analyze_media_with_ffprobe(self, filepath: str) -> tuple:
        ...
        try:
            cmd_video = [get_ffprobe_path(), "-v", "error", "-select_streams", "v:0", "-count_packets", "-show_entries", "stream=nb_read_packets", "-of", "default=noprint_wrappers=1:nokey=1", filepath]
            ...
            cmd_codec = [get_ffprobe_path(), "-v", "error", "-select_streams", "a:0", "-show_entries", "stream=codec_name", "-of", "default=noprint_wrappers=1:nokey=1", filepath]
            ...
            cmd_bitrate = [get_ffprobe_path(), "-v", "error", "-select_streams", "a:0", "-show_entries", "stream=bit_rate", "-of", "default=noprint_wrappers=1:nokey=1", filepath]
            ...
            if not bitrate_str or bitrate_str == "N/A":
                cmd_bitrate_fmt = [get_ffprobe_path(), "-v", "error", "-show_entries", "format=bit_rate", "-of", "default=noprint_wrappers=1:nokey=1", filepath]
            ...
            cmd_duration = [get_ffprobe_path(), "-v", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", filepath]
